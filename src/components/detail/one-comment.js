...
export default function OneComment({ comment }) {
    ...
    return (
        <li>
            <S.Wrapper>
                <S.InfoDataLow><UserCard><Avatar alt={user.login} src={user.avatar_url} />{user.login}</UserCard><div>{created_at === updated_at ? 'created_at' : 'updated_at'} {timeFormatter(updated_at)}</div></S.InfoDataLow>
                <div></div>
                <div></div>
